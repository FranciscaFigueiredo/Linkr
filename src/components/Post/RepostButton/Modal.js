...
export default function RepostModal({ post }) {
  const { myRepost, setMyRepost, modal, setModal } = useContext(PostContext);
  const user = JSON.parse(sessionStorage.getItem('user'));

  const question = myRepost
    ? 'Do you want to undo the re-post ?'
    : 'Do you want to re-post this link ?';

  function handleClick() {
    if (myRepost) {
      deleteRepost(user.token, post.id)
        .then(() => setMyRepost(false))
        .catch((err) => {
          console.log(err.response);
        });
    } else {
      repost(user.token, post.id)
        .then(() => setMyRepost(true))
        .catch((err) => {
          console.log(err.response);
        });
    }
